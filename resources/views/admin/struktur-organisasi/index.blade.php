...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('admin.struktur-organisasi.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded">+ Tambah</a>
</div>
                ...
                @foreach ($items as $item)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                            <a href="{{ route('admin.struktur-organisasi.edit', $item->id) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                            <form action="{{ route('admin.struktur-organisasi.destroy', $item->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
