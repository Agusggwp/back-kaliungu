...
<div class="space-y-4 md:space-y-6">
    ...
    <div class="bg-gradient-to-r from-emerald-500 to-cyan-500 rounded-lg md:rounded-2xl shadow-lg p-4 sm:p-6 text-white">
        <div class="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold flex items-center gap-2">👥 Struktur Organisasi</h1>
                <p class="text-emerald-100 mt-1 text-xs sm:text-sm md:text-base">Kelola struktur organisasi Banjar Kaliungu Kaja</p>
            </div>
    ...
    @if ($items->isEmpty())
        ...
        <div class="bg-white rounded-lg md:rounded-2xl shadow-lg p-6 md:p-8 text-center">
            <div class="flex flex-col items-center gap-3">
                ...
                <a href="{{ route('admin.struktur-organisasi.create') }}" class="mt-4 text-emerald-600 hover:text-emerald-800 font-medium text-sm md:text-base">Buat yang pertama →</a>
            </div>
        </div>
    @else
        <!-- Table Responsive -->
