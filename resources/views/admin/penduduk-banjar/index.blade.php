...
<div class="space-y-4 md:space-y-6">
    ...
    <div class="bg-gradient-to-r from-emerald-500 to-cyan-500 rounded-lg md:rounded-2xl shadow-lg p-4 sm:p-6 text-white">
        <div class="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold flex items-center gap-2">👥 Data Penduduk Banjar</h1>
                <p class="text-emerald-100 mt-1 text-xs sm:text-sm md:text-base">Kelola data demografis Penduduk Banjar Kaliungu Kaja</p>
            </div>
    ...
    @if ($items->isEmpty())
        ...
        <div class="bg-white rounded-lg md:rounded-2xl shadow-lg p-6 md:p-8 text-center">
            <div class="flex flex-col items-center gap-3">
                ...
                <a href="{{ route('admin.penduduk-banjar.create') }}" class="mt-4 text-emerald-600 hover:text-emerald-800 font-medium text-sm md:text-base">Buat data pertama →</a>
            </div>
        </div>
    @else
        <!-- Table Responsive -->
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div class="bg-gradient-to-br from-emerald-50 to-emerald-100 p-4 sm:p-6 rounded-lg md:rounded-xl border border-emerald-200 shadow-md">
                <p class="text-xs sm:text-sm font-semibold text-emerald-600 uppercase tracking-wide mb-1">Total Penduduk</p>
                <p class="text-2xl sm:text-3xl md:text-4xl font-bold text-emerald-900">{{ $items->sum('jumlah_laki_laki') + $items->sum('jumlah_perempuan') }}</p>
                <p class="text-xs sm:text-sm text-emerald-700 mt-1">orang</p>
            </div>
        </div>
    @endif
</div>
